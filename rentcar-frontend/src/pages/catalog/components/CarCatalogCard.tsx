...
interface Props { car: CarListItemDto }
...
export default function CarCatalogCard({ car }: Props) {
    ...
    return (
      <div
        ...
        style={{
          ...
          borderRadius:  14,
          overflow:      'hidden',
          border:        `1.5px solid ${hovered ? token.colorPrimary : token.colorBorderSecondary}`,
          cursor:        'pointer',
          boxShadow:     hovered
            ? `0 6px 20px rgba(22,119,255,0.14)`
      ...
      >
        {/* Image — left 38% */}
        <div style={{ position: 'relative', width: '38%', flexShrink: 0, background: token.colorFillSecondary }}>
          {imgSrc ? (
            <img
              src={imgSrc}
              alt={name}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', margin: '6px 0' }}>
            {[
              { icon: <SettingOutlined style={{ fontSize: 9 }} />, label: TRANSMISSION_LABEL[car.transmissionType] },
              { icon: <TeamOutlined    style={{ fontSize: 9 }} />, label: `${car.seatCount} o'rin` },
            ].map((s, i) => (
              <span key={i} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 9, color: token.colorTextTertiary }}>Kuniga</div>
              <div style={{ fontWeight: 800, fontSize: 15, color: token.colorPrimary, lineHeight: 1.1 }}>
                {fmt(car.dailyRate)}
                <span style={{ fontSize: 10, fontWeight: 500, color: token.colorTextSecondary, marginLeft: 2 }}>so'm</span>
              </div>
            </div>
            <div style={{
  ...
  return (
    ...
    >
      {/* Image */}
      <div style={{ position: 'relative', aspectRatio: '4 / 3', background: token.colorFillSecondary, overflow: 'hidden' }}>
        {imgSrc ? (
          <img
            src={imgSrc} alt={name}
            style={{
          ...
          <span style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontSize: 52 }}>🚗</span>
        )}
        ...
        <div style={{
          ...
          background: status.bg, backdropFilter: 'blur(6px)',
          border: `1px solid ${status.color}40`,
          fontSize: 11, fontWeight: 700, color: status.color, letterSpacing: 0.3,
        }}>
          {status.label}
        </div>
      ...
      <div style={{ padding: '12px 14px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {[
            ...
            <span key={i} style={{
              ...
              fontSize: 11, color: token.colorTextSecondary, whiteSpace: 'nowrap',
            }}>
        ...
        <div style={{
          ...
          borderRadius: 10, border: '1px solid rgba(22,119,255,0.12)',
        }}>
          <div>
            <div style={{ fontSize: 10, color: token.colorTextTertiary, marginBottom: 1 }}>Kuniga</div>
            <div style={{ fontWeight: 800, fontSize: 17, color: '#1677ff', lineHeight: 1 }}>
              {fmt(car.dailyRate)}
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 13, color: token.colorTextSecondary }}>{fmt(car.dailyRate * 10)} so'm</div>
          </div>
        </div>
